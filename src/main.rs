...
fn recv(stream: &UdpSocket, addr: &SocketAddr) -> std::io::Result<()> {
    ...
    match result {
        Ok(_) => {
            ...
            println!(
                "Name:     {}",
                &String::from_utf8_lossy(&buf[13..boundary]).to_owned()
            );
            ...
            println!(
                "TLD:      {}",
                &String::from_utf8_lossy(&buf[start..boundary]).to_owned()
            );
            ...
            let seconds: u32 =
                buf[boundary + 11] as u32 * 256 * 256 * 256 + buf[boundary + 12] as u32 * 256 * 256
                    + buf[boundary + 13] as u32 * 256 + buf[boundary + 14] as u32;
            println!(
                "TTL:      {} {} {} {} - {} seconds",
                &buf[boundary + 11],
                &buf[boundary + 12],
            ...
            println!("RDLENGTH: {} {}", &buf[boundary + 15], &buf[boundary + 16]);
            let octet_4 = buf[boundary + 17];
            let octet_3 = buf[boundary + 18];
            let octet_2 = buf[boundary + 19];
            let octet_1 = buf[boundary + 20];
            println!(
                "RDDATA:   {}.{}.{}.{}",
                &octet_4, &octet_3, &octet_2, &octet_1
            );

            if octet_4 == 0 && octet_3 == 0 && octet_2 == 0 && octet_1 == 0 {
                return Err(Error::new(ErrorKind::Other, "DNS failure"));
            }
        }
        Err(e) => {
        ...
        match result {
            ...
            Err(e) => {
                if count == retry_count {
                    println!(
                        "--> DNS failed after {} retries. Sending push... {}",
                        retry_count, e
                    );
                    send_push();
                    thread::sleep(sleep_millis);
